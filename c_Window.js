class Window {
  ...
  wline(x1, y1, x2, y2, dot) {
    if (dot) {
      ...
      if (y1==y2) {
        ...
        line(this.x+3*this.m+x1, height-this.m-y1, this.x+3*this.m+x2, height-this.m-y2);
  ...
  wcurve(a,  b,  c,  x1,  x2,  r) {
    for (var x = x1; x<x2; x+=2) {
      this.wline(x, a*sq(x)+b*x+c, x+2, a*sq(x+2)+b*(x+2)+c);
    }
  }

  warc(x1,  y1,  r) {
    for (var  x = x1; x<x1+r; x+=2) {
      this.wline(x, -sqrt(sq(r)-sq(x-x1))+y1, x+2, -sqrt(sq(r)-sq(x+2-x1))+y1);
    }
  }
  wpoint(x,  y,  r) {
