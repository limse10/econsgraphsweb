class Fill {
  ...
  render() {
    ...
    //this.identifyCurves();
    //this.psv=sortP(cleanP(this.psv));
  ...
  identifyCurves() {
    ...
    for (var i = 0; i<this.ps.length; i++) {
      if (this.ps[i].l1.n==2||this.ps[i].l2.n==2) {
        if (this.ps[i].l1==this.ps[(i+1)%(this.ps.length-1)].l1) {
        ...
        } else if (this.ps[i].l1==this.ps[(i+1)%(this.ps.length-1)].l2) {
          l = this.ps[i].l1;
          p1.x=this.ps[i].x;
          p1.y=this.ps[i].y;
          ...
          index=i;
          this.psv=insert(this.psv, curve, index);
        } else if (this.ps[i].l2==this.ps[(i+1)%(this.ps.length-1)].l1) {
          l = this.ps[i].l2;
          p1.x=this.ps[i].x;
          p1.y=this.ps[i].y;
          p2.x=this.ps[(i+1)%(this.ps.length-1)].x;
          ...
          index=i;
          this.psv=insert(this.psv, curve, index);
        } else if (this.ps[i].l2==this.ps[(i+1)%(this.ps.length-1)].l2) {
          l = this.ps[i].l2;
          p1.x=this.ps[i].x;
          p1.y=this.ps[i].y;
          p2.x=this.ps[(i+1)%(this.ps.length-1)].x;
