class Container {
  ...
  constructor(buttons, labels, icons, type) {
    ...
    if (this.type==0) {
      for (var i=0; i<buttons.length; i++) {
        this.buttons[i]=new Button(MAIN, labels[i], icons[i], u, (i+1.5)*1.4*u,u, (i+1.5)*1.4*u, u, color(190), color(170));
      ...
      for (var i=0; i<buttons.length; i++) {
        this.buttons[i]=new Button(SUB, labels[i],null, (1.3*i+2)*1.4*u, 0.8*u, (1.3*i+2)*1.4*u+0.8*u, 0.8*u, 0.8*u, color(200), color(150));
  ...
  render() {
    ...
    if (this.type==MAIN) {
      noStroke();
      fill(200);
      rect(this.x1-0.7*u, this.y1-0.4*u, 1.4*u, this.y2-this.y1+0.8*u);
